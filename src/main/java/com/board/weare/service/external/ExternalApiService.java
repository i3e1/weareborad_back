...
//        con = getLocationHttpCon(con, redirect);
        try {
            ...
            if(responseCode>=200 && responseCode<400){
                ...
                s = readBody(inputStream);
                throw new ExternalApiRequestException(apiUrl, s);
            }
            return s;
        } catch (IOException e) {
...
//            e.printStackTrace();
            ...
            throw new ExternalApiRequestException(apiUrl ,"외부 api 요청에 실패했습니다. responseCode : "+responseCode +"\nmessage : "+e.getMessage());
        } catch (Exception ex) {
            ex.printStackTrace();
            throw ex;
        } finally {
    ...
    private HttpURLConnection getLocationHttpCon(HttpURLConnection con, String redirect) {
        try {
            if (redirect != null) {
